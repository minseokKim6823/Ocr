...
## 부록: `vis_fields.py` — 특정 필드 빨간 박스 표시 (RapidOCR PoC)

문서 이미지에서 **'종목코드'·'종목명' 라벨 옆의 값만** 찾아 빨간 박스로 표시하는
독립 실행 스크립트입니다. 정형 양식에서 원하는 필드 위치를 빠르게 가시화·검증하는 용도입니다.

> **⚠️ 이 스크립트는 Gemma OCR과 무관합니다.**
> 위에서 설명한 본체(`ocr_gemma` / Gemma 4 / Ollama·Transformers·vLLM)와는 완전히
> 별개로, **RapidOCR(PP-OCRv5, 한국어)** 엔진을 사용합니다.
> Gemma 모델·Ollama·`ocr_gemma` 패키지를 전혀 호출하지 않으며,
> 의존성도 `rapidocr`, `opencv-python`, `numpy`만으로 독립적으로 동작합니다.
> 같은 저장소에 들어 있는 별도 PoC 도구일 뿐입니다.

### 실행

```bash
# jpg/ 폴더 전체 처리 (기본)
python vis_fields.py

# 한 장만 처리하고 결과를 기본 뷰어로 띄움
python vis_fields.py jpg/IRN1.jpg --open

# 글로브 패턴
python vis_fields.py "jpg/IRN*.jpg"

# 다른 라벨 지정 ('라벨 옆 값' 규칙 적용)
python vis_fields.py jpg/IRN1.jpg --fields "종목코드,발행회사"
```

처리된 이미지는 원본과 같은 위치에 `<원본이름>_fields.jpg`로 저장됩니다.
각 박스 위에는 영문 태그(`CODE`/`NAME`, 그 외 필드는 `F1`,`F2`…)가 표기됩니다
(OpenCV가 한글 텍스트를 그리지 못하기 때문).

### CLI 옵션

| 플래그 | 설명 | 기본값 |
|---|---|---|
| `images` | 파일/폴더/글로브 경로 (여러 개 가능, 없으면 `jpg/` 폴더 전체) | `jpg` |
| `--fields` | 찾을 라벨 목록 (쉼표 구분) | `종목코드,종목명` |
| `--box-thresh` | 텍스트 검출(det) 박스 임계값 | `0.3` |
| `--text-score` | 텍스트 인식(rec) 신뢰도 임계값 | `0.5` |
| `--open` | 이미지 1장일 때 결과를 기본 뷰어로 띄움 | 비활성화 |
| `--no-save` | 박스 그리기/이미지 저장 생략(값만 빠르게 추출) | 비활성화 |

### 동작 방식

- **빠른 경로**: 요청 필드가 `종목코드`/`종목명`뿐이면 검출(det) 1회 후
  '종목코드' 행만 타깃 인식해 속도를 높입니다. 코드 행을 못 찾으면 full 경로로 폴백합니다.
- **값 찾기 규칙**: 라벨이 단독 박스면 같은 줄 오른쪽 최근접 박스를 값으로,
  라벨+값이 한 박스면 라벨 접두를 제거해 값만 추출합니다.
- **종목명 폴백**: 회색 '(종목명)' 라벨 인식이 불안정하여, 인식 실패 시
  '종목코드' 값 기준 같은 줄 오른쪽의 가장 오른쪽 값 박스로 폴백합니다.
- **unclip 재시도**: 양식마다 최적 `unclip_ratio`가 달라 1.6 → 2.0 순으로
  못 찾은 필드만 재시도합니다.
- 실행 후 필드별 성공 건수와 OCR/저장 소요 시간 요약을 출력합니다.

> 참고: 같은 디렉터리의 `vis_boxes.py`는 필드 선별 없이 RapidOCR가 검출한
> 모든 텍스트 박스를 그려 검출 결과 전반을 확인할 때 사용합니다.
